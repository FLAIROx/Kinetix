...
def make_kinetix_env(
    ...
    make_empty_reset_func: bool = False,
    ...
    Args:
        config (Optional[dict]): The configuration dictionary generated by hydra, after being passed through `normalise_config`
        env_params (EnvParams): EnvParams
        static_env_params (StaticEnvParams): StaticEnvParams
        make_empty_reset_func (bool, optional): If true, the reset function is undefined, meaning that the environment will error when you do not pass a `reset_state` to `.step` and `.reset`. Defaults to False.
        ...
        reset_func (Callable[[chex.PRNGKey], EnvState], optional): If this is given, this is the function that gets called on reset to provide the starting state for the next episode. Defaults to None.
    ...
    if reset_func is None:
        if not make_empty_reset_func:
            assert (
                config is not None
            ), "If reset_func is not given and `make_empty_reset_func` is False, config must be given"
            reset_func = make_reset_func_from_config(
                config | ({} if "env_size_name" in config else {"env_size_name": "custom"}),
                physics_engine,
                env_params,
                static_env_params,
            )
